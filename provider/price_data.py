...
def insert_stock_prices(db, headers, base_url):
    try:
        ...
        # Get yesterday's date
        from_ = datetime.today() - relativedelta(months=60)
        ...
        for number, symbol_tuple in enumerate(symbols, 1):
            ...
            while True:
                url = f"{base_url}/v2/stocks/{symbol}/bars?&timeframe=1M&start={formatted_from}&end={formatted_to}&adjustment=raw&feed=sip&sort=asc"
                if page_token:
...
def fetch_incremental_stock_prices(db, headers, base_url,symbol=None):
    try:
        ...
        for symbol, latest_dt in symbol_dates:
            ...
            url = f"{base_url}/v2/stocks/{symbol}/bars?timeframe=1M&start={formatted_next_day}&end={formatted_today}&adjustment=raw&feed=sip&sort=asc"
            response = requests.get(url, headers=headers)
            response = requests.get(url, headers=headers)
            print("Response Status Code:", response.status_code)
            if response.status_code != 200:
